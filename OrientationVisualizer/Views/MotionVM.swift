...
final class MotionVM: ObservableObject {
    ...
    func start(updateHz: Double, demo: Bool? = nil) {
        stop()
        ...
        demoTask = Task { [weak self] in
            ...
            while !Task.isCancelled {
                ...
                await MainActor.run {
                    ...
                    self.pitchDeg =  self.lowPass(current: r, previous: self.pitchDeg)
                    self.yawDeg =  0
...
private extension Double { var toDegrees: Double { self * 180 / .pi } }
